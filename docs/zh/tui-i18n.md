...
![实时转写界面，左侧显示说话人，右侧显示正在生成的日文字幕](/screenshots/live-transcription.png)

按 `s` 打开设置，无需退出 TUI 即可调整界面语言、语音识别、AI、音频、共享和 VAD 选项。

![设置面板，可调整界面语言、语音识别、AI 和音频选项](/screenshots/settings.png)
...
在 **设置 → 语音识别 → 识别模型** 中按 `←` / `→`，可以在 SenseVoice 与
Fun-ASR-Nano 之间切换。若本地缺少 Fun-ASR-Nano，可选择留在下载界面，或让它
...
还是两个模型都安装。无人值守安装可使用 `--yes`，它会保留当前配置的识别模型并跳过询问。
