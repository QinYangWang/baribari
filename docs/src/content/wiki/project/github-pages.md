...
---
This site is a [Veka](https://github.com/masmuss/veka)-based Astro wiki under `docs/`, published with GitHub Actions.

## Locales

| Path | Language |
|------|----------|
| `docs/src/content/wiki/**` | English (default) |
...
| Item | Value |
|------|--------|
| Source | `docs/` (Astro / Veka) |
| Workflow | `.github/workflows/docs.yml` |
| Typical URL | `https://qinyangwang.github.io/baribari/` |
| `base` | `/baribari/` (project Pages) |

If the site is ever served from a **custom domain** or **user site**, change `BASE` in `docs/src/lib/site-config.ts` to `""` or `/` as appropriate.
...
```bash
npm run docs:install   # once: install docs/ dependencies
npm run docs:dev       # http://localhost:4321/baribari/
npm run docs:build     # output: docs/dist
...
2. `npm install` in `docs/`.
3. `npm run build` (Astro + Pagefind search index).
4. Upload `docs/dist` as a Pages artifact.
...
- Add a Markdown file under `docs/src/content/wiki/<category>/`.
...
- Put images in `docs/public/` (for example `docs/public/screenshots/`).
